 ...
 *
 * confirm_start_date() creates the WORK_START row at amount_kurus=0 because
 * the platform doesn't decide a price at that point -- pricing happens here,
 * by hand, until it is decided policy rather than a per-contract admin call.
 * Only a PENDING invoice can be repriced: once it is PAID or CANCELLED the
 * amount is history, not a draft.
 */
...
/** Marking a platform invoice paid or cancelled -- the only two moves an admin gets here. */
