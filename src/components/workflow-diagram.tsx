        ...
        <div className="fade-in relative overflow-hidden rounded-2xl border border-indigo-200/80 bg-gradient-to-br from-indigo-50/80 via-white to-purple-50/40 p-6 shadow-sm backdrop-blur-md dark:border-indigo-900/60 dark:from-indigo-950/40 dark:via-zinc-900 dark:to-zinc-900">
          ...
          <ol className="mt-6 grid gap-4 sm:grid-cols-3">
            {phases.map((p) => (
              <li
                key={p.id}
                className={`relative flex flex-col gap-2 rounded-xl bg-white/90 p-4 border shadow-2xs dark:bg-zinc-900/90 ${
                  p.is_completed
              ...
              >
                ...
                <p className="text-[0.75rem] text-zinc-500 dark:text-zinc-400 line-clamp-2">
                  {p.description || "Açıklama yok"}
                </p>
                {p.estimated_days && (
                  <p className="mt-1 text-[0.65rem] font-medium text-indigo-600 dark:text-indigo-400">
                    ~{p.estimated_days} gün
                  </p>
                )}
                {side === "freelancer" ? (
                  <PhaseNotesForm
                  ...
                  <PhaseNotesReadOnly notes={p.notes} />
                )}
